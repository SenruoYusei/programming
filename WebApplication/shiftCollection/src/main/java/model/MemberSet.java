...
public class MemberSet extends ArrayList<Member>{
	...
	public String[] getSubmittedSchedule(int m, int t) {
		...
		for(int i = 0;i < data.length;i++){
			schedule[i] = firstMember.getName() + "," + (( firstMember.getMonth() != m || firstMember.getTerm() != t) ? "" : data[i]) + "\n";
		}
		for(int i = 1;i < 30;i++) {
			Member member = (i < size()) ? get(i) : null;
			
			String[] submittedData = (member == null || member.getMonth() != m || member.getTerm() != t) ? new String[16] : member.getAllSchedule();
			for(int j = 0;j < submittedData.length;j++) {
				schedule[j] += (member == null ? "" : member.getName()) + "," + (submittedData[j] != null ? submittedData[j] : "") + "\n";
			}
		}
		return schedule;
	}
